...
export const Route = createFileRoute("/photography/stories")({
  ...
  loader: async () => {
    const stories = await client.fetch(`*[_type == "photographyStory"] | order(date desc) {
      "id": _id,
      title,
      description,
      date,
...
// A special component that handles opening and closing the album
  ...
  return (
    <FadeIn delay={index * 0.05}>
      <article className="overflow-hidden rounded-2xl border border-border bg-card/70 backdrop-blur-md transition-colors hover:border-foreground/40">
        ...
        >
          ...
          <div className="p-5 md:col-span-7 md:p-6 flex flex-col justify-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {story.date ? new Date(story.date).getFullYear() : ""}
            </p>
        ...
        {/* The Expandable "Inside the Album" Section */}
        <AnimatePresence>
          {isOpen && (
            <motion.div
            ...
            >
              <div className="p-6 md:p-8 border-t border-border bg-card/40">
                ...
                {/* Grid of the individual photos inside the story */}
